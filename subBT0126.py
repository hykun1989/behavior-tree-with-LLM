...
#     raw_string = str(response['choices'][0]['text'])
    ...
    output_text = raw_string.replace('\'', '').replace('\\n', '\n').replace('\\t', '\t')
    conditions, actions = parse_input(output_text)
    save_to_file(full_path, conditions, actions)
...
if __name__ == '__main__':
    ...
    final_prompt  = read_prompt("./prompt.txt") 
